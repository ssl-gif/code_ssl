...
1.定义一个Thread类的子类，并重写Thread类的run()方法，设置线程任务(即开启这个线程是用来做什么的?)
2.创建Thread类的子类对象，通过子类对象调用Thread类中的start()方法，启动线程。

    我们在main方法中，使用start()方法启动一个线程后，JVM会调用该线程的run()方法执行线程任务，结果是两个线程
(main、MyThread线程)并发地运行

#.注意事项：

(1).java程序属于抢占式调度，main、MyThread线程每次都抢占式的并发运行，所以下面程序的执行结果每次都不一样
...
public class DemoMain {
    public static void main(String[] args) {
        ...
        MyThread myThread = new MyThread("自己创建的第一个线程类");
