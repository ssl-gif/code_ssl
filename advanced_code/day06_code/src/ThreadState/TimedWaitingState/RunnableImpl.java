...
#.使用Object类中带参的wait()方法也可使线程进入计时等待状态。同sleep方法一样，wait方法计时结束后，线程就会进入可运行、锁阻
塞状态。但是wait(参数)方法还有以下用途：

    若wait()方法时间未结束，其他线程可以调用Object类中的notify()方法将其从休眠状态唤醒进入可运行、锁阻塞状态。但此时必须
使用同一个对象锁来实现线程之间的通信。具体实现原理与代码，和无限等待状态的"等待唤醒案例"一样。
